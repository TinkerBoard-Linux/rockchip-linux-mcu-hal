...
/**
 * @brief  Get GPIO Virtual state.
 * @param  pGPIO: The pointer of GPIO struct.
 * @return HAL_FuncStatus.
 */
HAL_FuncStatus HAL_GPIO_GetVirtualModeEn(struct GPIO_REG *pGPIO)
{
    return (pGPIO->GPIO_VIRTUAL_EN &
            GPIO_GPIO_VIRTUAL_EN_GPIO_VIRTUAL_EN_MASK) ?
           HAL_ENABLE : HAL_DISABLE;
}

/**
 * @brief  Get GPIOs in vmodel.
 * @param  pGPIO: The pointer of GPIO struct.
 * @param  vmode: The value defined in @ref eGPIO_VirtualModel.
 * @param  pin: The pin bit defined in @ref ePINCTRL_GPIO_PINS.
 * @return HAL_Status.
 */
HAL_Status HAL_GPIO_GetPinsInVirtualMode(struct GPIO_REG *pGPIO,
                                         eGPIO_VirtualModel vmodel,
                                         ePINCTRL_GPIO_PINS *pin)
{
#if (GPIO_VER_ID >= 0x01000C2BU)
    /* Support OS_A and OS_B */
    if (HAL_GPIO_GetVirtualModeEn(pGPIO) == HAL_DISABLE) {
        return HAL_ERROR;
    }

    ePINCTRL_GPIO_PINS status =
        (pGPIO->GPIO_REG_GROUP_H << 16) | pGPIO->GPIO_REG_GROUP_L;
...
/**
 * @brief  Update GPIOs vmodel.
 * @param  pGPIO: The pointer of GPIO struct.
 * @param  vmode: The value defined in @ref eGPIO_VirtualModel.
 * @param  pin: The pin bit defined in @ref ePINCTRL_GPIO_PINS.
 * @param  pin_mask: The pin bit defined in @ref ePINCTRL_GPIO_PINS.
 * @return HAL_Status.
 */
HAL_Status HAL_GPIO_UpdateVirtualMode(struct GPIO_REG *pGPIO,
                                      eGPIO_VirtualModel vmodel,
                                      ePINCTRL_GPIO_PINS pin,
                                      ePINCTRL_GPIO_PINS pin_mask)
{
    ...
    mask_low = (pin_mask & 0x0000ffff) << 16;
    mask_high = pin_mask & 0xffff0000;

    if (vmodel == GPIO_VIRTUAL_MODEL_OS_B) {
        pin = ~pin;
    }

    pGPIO->GPIO_REG_GROUP_L = mask_low | (pin & 0x0000ffff);
    pGPIO->GPIO_REG_GROUP_H = mask_high | ((pin & 0xffff0000) >> 16);
