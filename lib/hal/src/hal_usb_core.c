...
HAL_Status USB_ActivateDedicatedEndpoint(struct USB_GLOBAL_REG *pUSB, struct USB_OTG_EP *pEP)
{
    ...
    if (pEP->isIn == 1) {
        ...
        debug |= ((pEP->maxPacket & USB_OTG_DIEPCTL_MPSIZ) | (pEP->type << 18) |
                  ((pEP->num) << 22) | (USB_OTG_DIEPCTL_SD0PID_SEVNFRM) |
                  (USB_OTG_DIEPCTL_USBAEP));
        ...
        if (((USB_OUTEP(pEP->num)->DOEPCTL) & USB_OTG_DOEPCTL_USBAEP) == 0) {
            ...
            debug = (uint32_t)(((uint32_t)pUSB) + USB_OTG_OUT_ENDPOINT_BASE +
                               (0) * USB_OTG_EP_REG_SIZE);
            debug = (uint32_t)&USB_OUTEP(pEP->num)->DOEPCTL;
            debug |= ((pEP->maxPacket & USB_OTG_DOEPCTL_MPSIZ) | (pEP->type << 18) |
                      ((pEP->num) << 22) | (USB_OTG_DOEPCTL_USBAEP));
        }
