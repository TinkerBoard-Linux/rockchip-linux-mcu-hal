 ...
 *  @{
 ...
 The pinctrl driver provides APIs:
   ...
   - HAL_PINCTRL_SetPinParam to set pin iomux/pull/drive strength/slew rate/schmitt trigger
...
{                                                                      \
    HAL_DBG("pinctrl: write val = 0x%lx to register %p\n", VAL, &REG); \
    REG = VAL;                                                         \
    HAL_DBG("pinctrl: readback register %p = 0x%lx\n", &REG, REG);     \
}
